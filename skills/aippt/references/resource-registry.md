...
| Stage 0 / 6 | `style-auto-routing.md` | preset recommendation heuristics |
| Stage 0 / 6 | `config/preferences-schema.md` | project/user default configuration schema |
...
| Stage 4 / 5 | `layout-gallery.md` | slide archetypes and layout_hint vocabulary |
| Stage 5 | `bento-grid-system.md` | canonical layout geometry |
| Stage 5 | `page-plan-schema.md` | per-slide HOW contract |
| Stage 6 / 7 | `style-instruction-schema.md` | render-facing style instruction block |
...
| `scenes/*.json` | scene-specific defaults for sections, story arc, style bias, delivery default, density bias, layout tendency, review bias |
...
| File | Purpose |
| --- | --- |
| `styles/index.json` | style preset registry, keywords, and dimension mapping |
...
| Script | Purpose |
| --- | --- |
| `scripts/list-catalog.mjs` | list scene packs, style presets, delivery modes, validators, and preference paths |
| `scripts/init-workspace.mjs` | initialize `output/` tree and `output/project.json` |
| `scripts/read-preferences.mjs` | resolve project/user defaults from `EXTEND.json` |
...
| `scripts/_shared.mjs` | shared parsing, preferences, and style-instruction helpers |
...
- Stage 5: consult `resource-menu.md` and `layout-gallery.md` to avoid repetitive layouts and proof walls.
- Stage 6/7: keep `style-vocabulary.md`, `style-dimensions.md`, and `style-instruction-schema.md` aligned.
